 ...
 *
 * maxAge подобран по одному правилу: билет обязан пережить сессию, которую
 * он страхует. Иначе получается худший исход — кука протухла, баннер исчез,
 * а `sb-*` сессия цели жива, и человек молча продолжает сидеть в чужом
 * кабинете. Сессия Supabase живёт ~400 дней (столько стоит на sb-куке),
 * поэтому билету дан тот же срок.
 *
 * Короткий срок ничего не защищал бы: безопасность держится на привязке к
 * session_id (см. targetSessionId), а не на скорости протухания. Билет без
 * совпадающей сессии бесполезен, сколько бы он ни лежал.
 */
