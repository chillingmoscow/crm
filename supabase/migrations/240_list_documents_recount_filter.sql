...
-- ── Права ───────────────────────────────────────────────────────────────────
--
-- Вместе со старой функцией теряются и её права, а новая создаётся с
-- умолчанием PostgreSQL, где EXECUTE есть у PUBLIC. Миграция 207 это
-- умолчание снимала намеренно — повторяем, иначе список актов смог бы дёргать
-- кто угодно, включая будущие роли базы.
--
-- anon намеренно ОСТАВЛЕН в списке, хотя функция ему ничего не вернёт (без
-- активного аккаунта выборка пуста). Попытка отозвать у него право приводит к
-- падению сервера: на локальном образе Supabase (PostgreSQL 17.6) вызов
-- функции ролью без EXECUTE роняет backend с signal 11 вместо отказа прав.
-- Проверено: с грантом anon получает 0 строк, без гранта — segfault; то же
-- воспроизводится на list_my_permissions, то есть дело не в этой функции.
-- Прод пока на 15.8, и проверять там такое нельзя — падение перезапустит базу
-- и оборвёт все соединения. Итог: восстанавливаем ровно тот набор прав,
-- который был до миграции, и снимаем только PUBLIC.
