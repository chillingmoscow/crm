-- Обновление функции инициализации пользователя с правильными бизнес-данными
-- Создает: Моя компания, Мой первый бизнес, должность Владелец, счета, группы контрагентов и статьи операций
...
DECLARE
  ...
  v_full_name TEXT;
  v_suppliers_group_id UUID;
  v_landlords_group_id UUID;
  v_contractors_group_id UUID;
  v_result JSON;
BEGIN
  ...
  -- 7. Обновляем пользователя с ID организации, должности и юрлица
  ...
  VALUES (
    'Поставщики',
    'Все, кто поставляет товары/услуги',
    v_org_id,
    v_user_id,
    NOW()
  ) RETURNING id INTO v_suppliers_group_id;
  ...
  VALUES (
    ...
    v_user_id,
    NOW()
  ) RETURNING id INTO v_landlords_group_id;

  -- Подрядчики
  INSERT INTO public.counterparty_groups (name, description, organization_id, created_by, created_at)
  VALUES (
    'Подрядчики',
    'Юристы, клинеры, бухгалтеры и т.д.',
    v_org_id,
    v_user_id,
    NOW()
  ) RETURNING id INTO v_contractors_group_id;
  ...
  ('Доход от продажи', 'income', '#4CAF50', v_org_id, v_legal_entity_id, v_user_id, NOW()),
  ('Прочие доходы', 'income', '#2196F3', v_org_id, v_legal_entity_id, v_user_id, NOW());

  -- Категории расходов
  INSERT INTO public.categories (name, type, color, organization_id, legal_entity_id, created_by, created_at) VALUES
  ('Актуализация', 'expense', '#FF5722', v_org_id, v_legal_entity_id, v_user_id, NOW()),
  ('Зарплата персонала', 'expense', '#9C27B0', v_org_id, v_legal_entity_id, v_user_id, NOW()),
  ('Закупка товаров', 'expense', '#FF9800', v_org_id, v_legal_entity_id, v_user_id, NOW()),
  ('Аренда помещения', 'expense', '#795548', v_org_id, v_legal_entity_id, v_user_id, NOW()),
  ('Коммунальные услуги', 'expense', '#607D8B', v_org_id, v_legal_entity_id, v_user_id, NOW()),
  ('Ремонт и обслуживание', 'expense', '#FFC107', v_org_id, v_legal_entity_id, v_user_id, NOW()),
  ('Налоги и сборы', 'expense', '#F44336', v_org_id, v_legal_entity_id, v_user_id, NOW()),
  ('Штрафы и санкции', 'expense', '#E91E63', v_org_id, v_legal_entity_id, v_user_id, NOW());

  RAISE LOG '✅ Созданы категории операций';

  -- 10. Создаем счета
  -- Расчетный счет (банковский)
  INSERT INTO public.accounts (
    ...
    0.00,
    'RUB',
    'Расчетный счет для ведения основной деятельности',
    'checking',
    v_org_id,
    v_legal_entity_id,
    v_user_id,
  ...
  INSERT INTO public.accounts (
    ...
    'Для ежедневной выручки',
    0.00,
    'RUB',
    'Касса для ежедневных операций и выручки',
    'cash',
    v_org_id,
    v_legal_entity_id,
    v_user_id,
  ...
  -- Сейф (долгосрочное хранение)
  INSERT INTO public.accounts (
    name, balance, currency, description, account_type,
    organization_id, legal_entity_id, created_by, created_at
  ) VALUES (
    'Долгосрочное хранение налички',
    0.00,
    'RUB',
    'Сейф для долгосрочного хранения наличных средств',
    'cash',
    v_org_id,
    v_legal_entity_id,
    v_user_id,
...
COMMENT ON FUNCTION public.init_user_full_setup IS 'Обновленная функция инициализации пользователя с полным набором бизнес-данных согласно ТЗ'; 