-- ============================================================
-- 102_kb_notif_payload_page_icon.sql
--
-- Refresh KB-эмиттеров с добавлением page_icon + page_icon_color в
-- payload — для рендера entity-chip'а в bell'е (как KbPageMention в
-- editor'е). Без этого все chip'ы рендерятся с дефолтным FileText'ом.
--
-- Sprint E §2 polish, миграция 102. Заменяет миграции 094/095/100
-- (CREATE OR REPLACE — идемпотентно).
-- ============================================================

-- ============================================================
-- 1. kb_notify_required_reading
-- ============================================================
...
begin
  ...
  select t.id, t.created_by, t.page_id, p.slug, p.title, p.icon, p.icon_color, p.account_id
    into v_thread
  ...
  then
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'page_icon', v_thread.icon,
        'page_icon_color', v_thread.icon_color
      )
    );
  ...
  -- 2. Previous commenters (Codex #84 P1 CTE chain).
  with eligible as (
...
begin
  ...
  select id, slug, title, icon, icon_color, account_id, created_by, plain_text
    into v_page
  ...
  loop
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'page_icon', v_page.icon,
        'page_icon_color', v_page.icon_color
      )
    );
  ...
  loop
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'page_icon', v_comment.page_icon,
        'page_icon_color', v_comment.page_icon_color,
