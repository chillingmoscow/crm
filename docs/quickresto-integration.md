...
> **Ретраи при сетевой деградации.** Маршрут прода → `id.quickresto.ru`
> периодически деградирует (см. Граблики №6), поэтому все 4 шага KC-входа
> (и только они — data-вызовы не ретраятся, чтобы не повторять
> неидемпотентные POST/DELETE) ходят через `quickRestoFetch` c retry:
> 3 попытки с паузами 1с/3с на **pre-delivery** сетевые ошибки
> (`ENOTFOUND`, `ECONNREFUSED`, `ECONNRESET`, `ETIMEDOUT`,
> `UND_ERR_CONNECT_TIMEOUT` и др.). HTTP-ответы (в т.ч. 5xx), TLS-ошибки и
> собственный 20с-таймаут не ретраятся.
...
6. **Сетевая деградация прода → `id.quickresto.ru` (77.95.133.244)** —
   маршрут с прода (Timeweb, СПб) к Keycloak-хосту периодически теряет
   40–60% пакетов, TCP-коннект то подвисает, то проходит за 15мс
   (симптомы: `fetch failed` / `UND_ERR_CONNECT_TIMEOUT` в мастере,
   «не удалось соединиться с id.quickresto.ru»). Это деградация маршрута,
   а не баг кода; лечится ретраями KC-входа (`quickRestoFetch`, 3 попытки)
   и/или временем (окна бывают минутами). Диагностика с прода:
   `mtr -r -c 10 id.quickresto.ru`, `curl -sv --max-time 5 https://id.quickresto.ru/`.
   Альтернативных хостов Keycloak на стороне QR нет
   (`keycloak/sso/auth.quickresto.ru` → `.254` — другой сервис).
7. **Исторические грабли больше не актуальны** (оставлены для контекста):
