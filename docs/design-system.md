...
ГочА: обёртка `overflow-x-auto` (нужна для resize-колонок) делает контейнер
скролл-областью и по вертикали — поэтому `position: sticky; top: 0` на
`<thead>` липнет к этой обёртке, а не к окну. Решение — сделать обёртку
**вертикальным** скролл-контейнером и закрепить шапку внутри неё:

- обёртка таблицы: `max-h-[calc(100dvh-15rem)] overflow-auto` (вместо
  `overflow-x-auto`). `15rem` — стандартный резерв под топбар/заголовок/
  контролы; единое значение по всем таблицам ради консистентности;
- `<thead>`: `sticky top-0 z-20` + **непрозрачный** фон на ячейках
  (`bg-muted [&_th]:bg-muted`, не `bg-muted/60` — иначе строки просвечивают
  сквозь шапку).

Прецеденты: список «Акты инвентаризации», «Итоги» акта.

Для не-табличных списков (форма заполнения акта — карточки, скролл оконный)
overflow-ловушки нет: «шапку» (контролы + прогресс) закрепляем напрямую
