-- 212_inventory_status_sort_lifecycle.sql
--
-- Сортировка списка актов по статусу шла по тексту английского кода
-- (d.status::text) → для пользователя порядок выглядел случайным
-- (Назначен, В работе, Проведен, Готов…). Делаем сортировку по
-- ЖИЗНЕННОМУ ЦИКЛУ через числовой ранг status_rank:
--   Новый → Назначен → В работе → Готов к проверке → На пересчёте →
--   Итоги требуют проверки → Проведен → Ошибка синхронизации.
--
-- Return-type не меняется (status_rank не входит в returns table, только во
-- внутренние CTE) → достаточно CREATE OR REPLACE, без DROP и повторного GRANT.
-- search_path задаётся в теле (см. feedback_create_or_replace_search_path).
