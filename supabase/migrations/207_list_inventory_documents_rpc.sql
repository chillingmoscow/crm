...
-- inbox-сортировкой и пагинацией.
--
-- Зачем не PostgREST: нужна вычисляемая колонка zone_priority
-- (inbox-сортировка), поиск по document_items.product_name через
-- EXISTS, агрегация matched_ingredients для UI-хайлайта. Всё это
-- в PostgREST либо невозможно, либо требует склейки 2-3 запросов
-- на клиенте. RPC даёт один round-trip и переиспользует RLS
-- (security invoker → политика documents_select / document_items_select
-- из миграции 195 применяется автоматически).
--
-- Параметры:
...
-- строки или 0, если строк нет (отдельный запрос на пустой total
-- слишком дорог из-за повторного применения RLS).
-- ============================================================
...
declare
  ...
  v_can_manage boolean := public.has_permission('inventory.manage_documents');
  ...
  v_sort text := coalesce(nullif(p_sort, ''), 'inbox');
begin
  ...
  with filtered as (
    select
      ...
      d.comment,
      case
        when d.assigned_to = v_user_id and d.status in ('assigned', 'in_progress') then 1
        when v_can_manage and d.status = 'ready_for_review' then 1
        when d.status in ('synced', 'assigned') then 2
        when d.status = 'in_progress' then 3
        else 4
    ...
    where d.account_id = v_account_id
      ...
      -- date range (inclusive с обоих концов; для date_to добавляем один день,
      -- если invoice_date это timestamptz — у нас date, оставляем lte)
  ...
  ordered as (
    ...
    order by
      case when v_sort = 'inbox'       then zone_priority end asc  nulls last,
      case when v_sort = 'inbox'       then invoice_date  end desc nulls last,
