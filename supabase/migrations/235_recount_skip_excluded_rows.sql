...
--    залежавшийся АВТО-флаг с неё снимается — в том числе когда акт на
--    пересчёте (см. комментарий у самой ветки: гард из 228 про пороги, а не
--    про исключения). Ручную пометку не трогаем: её ставил человек и мог
--    иметь причины помимо суммы.
--
...
begin
  ...
  --
  -- Условие `not v_on_recount` здесь СОЗНАТЕЛЬНО отсутствует, хотя в двух
  -- соседних ветках оно есть. Оно из миграции 228 и защищает от ПОРОГОВОГО
  -- снятия отметок: пока акт на пересчёте, синхронизация продолжает
  -- импортировать строки, QR пересчитывает остатки, расхождение уходит ниже
  -- порога — и отметки круга исчезали у исполнителя под руками.
  --
  -- Исключение строки порогом не управляется, это явное решение человека или
  -- правила. Оставить здесь `not v_on_recount` значило бы, что правило
  -- «Исключать всегда», применённое во время круга, оставляет строку
  -- одновременно исключённой из итогов и отмеченной на пересчёт — ровно то
  -- противоречие, ради которого миграция и пишется. Условия ортогональны.
