...
### Закрепление шапки при скролле (sticky header)

Для длинных таблиц шапка должна оставаться видимой. ГочА: обёртка
`overflow-x-auto` (нужна для resize-колонок) делает контейнер скролл-областью
и по вертикали — поэтому `position: sticky; top: 0` на `<thead>` липнет к этой
обёртке, а не к окну. Решение — сделать обёртку **вертикальным** скролл-
контейнером и закрепить шапку внутри неё:

- обёртка: `max-h-[calc(100dvh-15rem)] overflow-auto` (вместо `overflow-x-auto`);
...
`sticky top-0 z-20 bg-background`. Прецедент — PR-sticky на акте инвентаризации.
