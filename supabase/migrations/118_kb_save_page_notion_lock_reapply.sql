-- 118_kb_save_page_notion_lock_reapply.sql
-- Re-apply the Notion-style save semantics for environments where the
-- 117 function body was not applied together with the app deploy.
--
-- Important: this migration intentionally does NOT update kb_pages.locked_at.
-- It only guarantees that global lock remains a UI guard, while ordinary
-- edit permissions stay the backend security gate.
...
comment on function public.kb_save_page(uuid, text, text, text, jsonb, text, uuid[]) is
  'Saves a KB page using ordinary edit permissions. Global lock is a '
  'Notion-style UI guard and is not a backend save blocker.';
