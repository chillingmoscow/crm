...
**CTA не висит в шапке.** Раньше был «always-rendered + disabled когда не
надо», но это неэлегантно — кнопка занимала место и сбивала с толку
(«почему здесь Сохранить, если ничего не менялось?»). Правило:

- Save / иные form-actions живут в **футере формы конкретного таба**,
  правым выравниванием.
- Рендерятся только при `canEdit && dirty` — нет необходимости — нет
  кнопки. Когда у пользователя нет прав или нет изменений, ничего
  не отображается, не дезориентирует.
- Header остаётся стабильным при переключении табов (без layout-shift):
  заголовок и подзаголовок — единственное содержимое.

Если когда-нибудь у конкретной сущности появится действительно глобальное
действие, релевантное для всех табов (типа «Опубликовать» / «Архивировать») —
такие действия можно вернуть в шапку, но only when always-relevant.
