-- Исправление функции init_user_full_setup для правильного создания организации с owner_id
-- Миграция от 03.01.2025

-- Обновляем функцию init_user_full_setup с правильным порядком создания
...
BEGIN
  ...
  INSERT INTO public.organizations (
    name,
    description,
    owner_id,  -- ← ВАЖНО: указываем owner_id
    settings,
    is_active,
    created_at
  ) VALUES (
    ...
    v_user_id,  -- ← owner_id = созданный пользователь
    '{}'::jsonb,
    TRUE,
    NOW()
...
-- Принудительно отключаем RLS для таблицы users (на всякий случай)
ALTER TABLE public.users DISABLE ROW LEVEL SECURITY;

-- Комментарии
COMMENT ON FUNCTION public.init_user_full_setup IS 'Полная инициализация нового пользователя БЕЗ поля username (исправлена проблема с owner_id)'; 